...
public class RedisExecutor {
    ...
    /**
     * 选择使用数据库（默认选中第一个）
     * @param label 在spring.xml里配置的jdbc jedisPool
     * @return RedisExecutor
     */
    public static RedisExecutor use(String label) {
        RedisExecutor executor =new RedisExecutor();
        executor.pool = RedisKit.getJedisPool(label);
        return executor;
    }
    /**
     * 默认第一个jedisPool
    ...
    public static RedisExecutor use() {
        RedisExecutor executor = new RedisExecutor();
        executor.pool = RedisKit.getJedisPool();
        return  executor;
    }
